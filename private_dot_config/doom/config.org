...
(doom! :input
       ...
       ligatures ; enabled        ; ligatures and symbols to make your code pretty again
       ...
       (rust +lsp) ; enabled       ; Fe2O3.unwrap().unwrap().unwrap().unwrap()
       ;;scala             ; java, but good
...
#+end_src
** Other funny config for company and YASnippet :DISABLED:
