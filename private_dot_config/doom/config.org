...
Enable eldoc-buffer as a separated buffer (mostly used to program in rust)

#+begin_src emacs-lisp :tangle config.el
  (define-key
    rust-mode-map ; For now only enables it in rust mode, since I'm not using it outside of rust
    "\C-ce" 'eldoc-doc-buffer)
#+end_src
